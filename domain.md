...
- `survey/*.tex` 正文实际引用：90 个 key，全部能在 Bib 中找到。
- `reference/`：90 个论文/规范记录 README（不含 9 个目录索引或空 SoK 占位 README），73 个本地 `paper.pdf`。
...
| Attestation、boot、lifecycle | 已覆盖-基础 | `rats_rfc`, `eat_rfc`, `menetrey2022attestation`, `psa_certified`, `ling2021trustzoneatt`, `chen2024mraima`, `mao2025pdrima`, `seshadri2004swatt`, `defrawy2012smart`, `asokan2015seda`；Arm CCA vs RISC-V CoVE/AP-TEE claim-strength table 已明确 E0/E1 与 E3 边界 | `menetrey2022attestation` | Foundational `seshadri2004swatt`; Spec/standard SOTA `rats_rfc`, `eat_rfc`; Draft/not ratified `mao2025pdrima`; E3 `riscv_ap_tee_2024` | 待深化 verifier policy、runtime freshness 和 platform-specific evidence profile。 |
