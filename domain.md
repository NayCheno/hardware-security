...
- `survey/reference.bib`：99 个 active in-scope BibTeX 条目。
...
- 64 条 out-of-scope attack-only BibTeX 已从 active bibliography 拆出并保存在 `survey/excluded_attack_reference.bib`；后续若用户明确转向攻击 SoK，再以该文件作为独立 scope 的起点。

## 1. 知识覆盖矩阵

| 知识点 / 小方向 | 状态 | 正文覆盖依据 | SoK / Survey 锚点 | 代表 reference / evidence role | 下一步 |
|---|---|---|---|---|---|
| 硬件辅助 TEE 总体设计空间 | 已覆盖-基础 | `survey/tee_design_space.tex` 已在正文前部建立 launch/admission、runtime isolation、trusted I/O、secure storage、attestation/verifier policy、TCB/excluded attacks 的上位分类，并限定 SoK/survey 只作 taxonomy support | `schneider2022soktee`; `li2024sokteechoices`; `boubakri2025riscvtee`; `sok-tee` | Foundational `schneider2022soktee`; Background substrate `li2024sokteechoices`, `boubakri2025riscvtee`, `sok-tee` | 已足够作全局入口；投稿前只随新增平台维护 taxonomy bridge。 |
...
| Attestation、boot、lifecycle | 已覆盖-基础 | `rats_rfc`, `eat_rfc`, `menetrey2022attestation`, `psa_certified`, `ling2021trustzoneatt`, `chen2024mraima`, `mao2025pdrima`, `seshadri2004swatt`, `defrawy2012smart`, `asokan2015seda`, `delignatlavaud2025adns`；Arm CCA vs RISC-V CoVE/AP-TEE claim-strength table 已明确 E0/E1 与 E3 边界 | `menetrey2022attestation` | Foundational `seshadri2004swatt`; Spec/standard SOTA `rats_rfc`, `eat_rfc`; Peer-reviewed SOTA `delignatlavaud2025adns`; Draft/not ratified `mao2025pdrima`; E3 `riscv_ap_tee_2024` | 待深化 verifier policy、runtime freshness、attested naming 和 platform-specific evidence profile。 |
...
| ISA / 硬件设计防御: MTE、PTE/page table、CFI、capability/tagging | 已覆盖-基础 | `survey/security_of_hardware_design.tex` 已补 PTE/page-table permission、PAN/PXN/UXN、PMP/ePMP/Smepmp、Zicfiss/Zicfilp、CHERI/CHERIoT、RV-CURE、memory encryption/integrity、hardware isolation primitive comparison 和 memory tagging/authenticated encryption/ECC 层级关系 | 无单一 SoK；以 architecture specs 和硬件实现论文为准 | Spec/standard SOTA `armv-a`, `riscv_privileged`; Draft/not ratified `manoni2026cva6cfi`; Peer-reviewed SOTA `kim2023rvcure`, `amar2023cheriot`, `chen2024hardwareisolation`, `lamster2024voodoo`; Background substrate `henson2014memory` | 待深化 Arm MTE 与 CHERI/RV-CURE/Voodoo/MAGIC 的机制差异和开发者可用性比较。 |
...
| `pcisig_tdisp_2022`, `pcisig_tdisp_xt_2025` | `reference/memory-and-io-fabrics/pcie-tee-device-interface-security-protocol-tdisp/` | PCIe trusted device interface lifecycle；TDISP XT public metadata lists IDE/ATS/TDISP as affected PCIe 6.x areas；公开 PDF 受限，metadata 已记录 | 只作 TDISP/TDISP XT public metadata 和 CoVE-IO/TEE-I/O 机制说明；不要伪称本地 PDF 可用或推断 gated XT/TE 规则。 |
| `volos2018graviton` | `reference/accelerator-tees/graviton-trusted-execution-environments-gpus/` | GPU secure context、command-processor ownership、secure copy/kernel launch；本地 PDF 已下载并解析 | Foundational GPU TEE baseline；硬件机制为原型/emulation，不能写成 production GPU 标准。 |
| `hunt2020telekine` | `reference/accelerator-tees/telekine-secure-computing-cloud-gpus/` | Cloud GPU data-oblivious stream abstraction 和 API remoting；本地 PDF 已下载并解析 | 建立 GPU TEE runtime/API side-channel 边界；底层 GPU TEE 是前提假设。 |
| `mai2023honeycomb` | `reference/accelerator-tees/honeycomb-secure-efficient-gpu-executions-static-validation/` | Static-validation GPU TEE、validator/SVSM/sandbox monitor；本地 PDF 已下载并解析 | 用于 software/static-validation GPU TEE baseline；不要泛化到所有 GPU workload。 |
| `zhao2022shef` | `reference/accelerator-tees/shef-shielded-enclaves-cloud-fpgas/` | Cloud FPGA secure boot、attestation 和 configurable Shield；本地 PDF 已下载并解析 | 用于 FPGA accelerator TEE baseline；不替代 PCIe TDISP/CoVE-IO device lifecycle。 |
| `fan2025xputee` | `reference/accelerator-tees/xputee-high-performance-practical-heterogeneous-tee-gpus/` | CPU/GPU XpuEnclave metadata and abstract；本地 PDF 不可用 | E5/source-limited；仅作近年 heterogeneous GPU TEE baseline metadata，不作强机制或性能 claim。 |
| `xia2021sgxfpga` | `reference/accelerator-tees/sgx-fpga-trusted-execution-cpu-fpga-heterogeneous-architecture/` | CPU-FPGA TEE metadata；本地 PDF 不可用 | E5/source-limited；只作 CPU-FPGA lineage marker，不作强机制 claim。 |
| `perkins2024socsok` | `reference/accelerator-tees/sok/trusted-execution-soc-fpgas/` | SoC-FPGA TEE feature/gap taxonomy；本地 PDF 已下载并解析 | SoK evidence；primary mechanism claims still return to original systems. |
...
3. `sok-tee` — Background substrate；NDSS 2026。
...
1. `arm_trustzone_whitepaper` — Foundational；Arm whitepaper。
...
2. `pinto2019trustzone` — Background substrate；ACM CSUR 2019 survey。
...
3. `cerdeira2020trustzone` — Background substrate；IEEE S&P 2020 SoK。
...
1. `li2022cca` — Foundational；OSDI 2022。
...
2. `arm_cca_spec` — Spec/standard SOTA；Arm spec 2025。
...
3. `arm_rmm_spec` / `wu2024rmm` — Spec/standard SOTA plus Draft/not ratified verification evidence；Arm spec 2025 / arXiv 2024。
...
1. `zhang2023shelter` — Foundational；USENIX Security 2023。
...
2. `zhou2025rcontainer` — Peer-reviewed SOTA；NDSS 2025。
...
3. `liu2025nanozone` / `liu2025lesstrust` — Draft/not ratified plus Peer-reviewed SOTA evidence；arXiv 2025 / IEEE TIFS 2025。
...
1. `acai2023` — Foundational；arXiv 2023。
...
2. `bertschi2026devlore` — Draft/not ratified；arXiv 2024 / year-tag 2026。
...
1. `seshadri2004swatt` — Foundational；IEEE S&P 2004。
...
1. `riscv_privileged` — Foundational；RISC-V privileged spec。
...
2. `feng2021penglai` — Peer-reviewed SOTA；OSDI 2021。
...
3. `schrammel2023spearv` — Peer-reviewed SOTA；ACM AsiaCCS 2023。
...
1. `sahita2023cove` — Foundational；arXiv 2023。
...
3. `boubakri2025riscvtee` — Background substrate；Electronics 2025 survey。
...
1. `feng2024siopmp` — Foundational；ASPLOS 2024。
...
1. `henson2014memory` — Foundational；ACM CSUR 2014。
   - 内容摘要：综述内存加密既有技术。
...
1. `gouk2022directcxl` — Foundational；USENIX ATC 2022。
...
2. `zhong2024cxltiers` — Peer-reviewed SOTA；OSDI 2024。
...
3. `wang2025odrp` — Peer-reviewed SOTA；NSDI 2025。
...
1. `dmtf_spdm_2025` / `dmtf_spdm_arch_2024` — Foundational；DMTF specification / white paper。
...
2. `riscv_cove_io_2026` / `pcisig_tdisp_2022` / `pcisig_tdisp_xt_2025` — Draft/not ratified plus Candidate/metadata-only gated records。
   - 内容摘要：CoVE-IO 把 TVM trusted I/O 拆成 TDI/TDM/DSM、SPDM、TDISP、PCIe IDE、IOMMU、trusted MSI 等组合；TDISP 关注 PCIe trusted device interface lifecycle；TDISP XT public metadata only supports that IDE、ATS 和 TDISP areas are affected in a PCIe 6.x ECN。
...
1. `zhu2020hetee` — Foundational；IEEE S&P 2020。
...
2. `dhar2024cloudscale` — Peer-reviewed SOTA；ACSAC 2024。
...
3. `wang2026cage` / `sang2025portal` / `deng2022strongbox` — Peer-reviewed SOTA plus Background substrate device-path evidence。
   ...
   - 文章评价：CAGE/PORTAL 是 Arm CCA device/accelerator Peer-reviewed SOTA；StrongBox 是历史 baseline，不能写成 CCA Realm 机制。

### 3.15 SmartNIC / trusted NIC / secure storage data path

1. `zhou2024snic` — Peer-reviewed SOTA；EuroSys 2024。
...
2. `giantsidi2025tnic` — Peer-reviewed SOTA；ASPLOS 2025。
...
3. `chrapek2026hazel` — Draft/not ratified；arXiv 2026。
...
| 对象 | 核验结果 | 处理 |
|---|---|---|
| 正文 97 个 cite key | 全部存在于 `survey/reference.bib`。 | 无缺失 key；SGX runtime background entries are outside active bibliography. |
...
| `chen2024hardwareisolation`, `lamster2024voodoo` | USENIX ATC / USENIX Security PDFs 已下载并验证。 | 用于 hardware-isolation primitive comparison、memory tagging/authenticated encryption/ECC；不要写成 TEE/CCA/CoVE/trusted-I/O 机制。 |
...
| `pcisig_tdisp_2022`, `pcisig_tdisp_xt_2025` | PCI-SIG public metadata 已记录；public PDF/member-gated，未下载本地 PDF。TDISP XT public page lists a 2025-04-22 PCIe 6.x ECN affecting IDE、ATS、TDISP。 | 用于 trusted device interface lifecycle 和 XT affected-area metadata；正文需避免声称本地 PDF、完整规范内容或 XT/TE normative rules 已公开可核验。 |
...
| `delignatlavaud2025adns` | USENIX Security 2025 PDF 已下载并验证。 | 用于 attested service naming、policy discovery、certificate issuance 和 transparency log；不替代 device identity、DMA control 或 service semantic correctness。 |
...
| `chen2024cpc`, `kuhne2026aster` | USENIX ATC / MobiSys PDFs 已下载并验证。 | CPC 用于 CVM maintenance/lifecycle；Aster 用于 Android AVF-on-CCA deployment。二者都不是 Arm 官方 interface/profile 或 production availability proof。 |
| `deng2022strongbox` | ACM CCS 2022 PDF 已下载并验证。 | 用于 Arm endpoint GPU TEE baseline；不是 CCA/Realm 机制。 |
| `volos2018graviton`, `hunt2020telekine`, `mai2023honeycomb`, `zhao2022shef`, `perkins2024socsok` | USENIX / ASPLOS / arXiv PDFs 已下载并验证。 | 用于 GPU/FPGA/SoC-FPGA accelerator TEE baseline 和 taxonomy；机制 claim 必须按各自 threat model 写。 |
| `fan2025xputee`, `xia2021sgxfpga` | DOI/metadata 页面已核验；公开 PDF 下载分别返回 403 / 418。 | E5/source-limited；仅作 baseline metadata 和 lineage marker，不作强机制或性能 claim。 |
| `zhou2024snic`, `giantsidi2025tnic`, `khalilov2024osmosis` | EuroSys / ASPLOS / USENIX ATC PDFs 已下载并验证。 | 用于 SmartNIC/NIC root-of-trust、secure offload 和 resource-management substrate；需要和 VM/Realm attestation、SPDM/TDISP、DMA protection 组合；OSMOSIS 不支撑 confidentiality claim。 |
...
- 所有 draft/spec materials 必须显式写状态，尤其是 `riscv_ap_tee_2024` 和 `riscv_cove_io_2026`。
...
| 已完成初稿 | 补 memory protection 与 ISA / 硬件设计防御 taxonomy，覆盖 PTE/page-table permission、MTE/PAC/BTI/GCS、PMP/ePMP/Smepmp、Zicfiss/Zicfilp、CHERI/CHERIoT、RV-CURE、hardware isolation comparison、memory tagging、memory encryption/integrity/replay protection。 | `armv-a`, `riscv_privileged`, `henson2014memory`, `manoni2026cva6cfi`, `kim2023rvcure`, `amar2023cheriot`, `chen2024hardwareisolation`, `lamster2024voodoo` |
...
| 已完成初稿 | 继续补 Arm CCA device access、GPU/FPGA accelerator、SmartNIC/NIC root-of-trust、SmartNIC resource-management substrate 与 secure storage data path 相关工作。 | `sang2025portal`, `wang2026cage`, `deng2022strongbox`, `zhou2024snic`, `giantsidi2025tnic`, `khalilov2024osmosis`, `chrapek2026hazel` |
| 已完成初稿 | 晋升 accelerator TEE baseline，补 Graviton、Telekine、Honeycomb、ShEF、XpuTEE、SGX-FPGA 和 SoC-FPGA SoK；XpuTEE/SGX-FPGA 均保留 source-limited 状态。 | `volos2018graviton`, `hunt2020telekine`, `mai2023honeycomb`, `zhao2022shef`, `fan2025xputee`, `xia2021sgxfpga`, `perkins2024socsok` |
| 已完成初稿 | 在 confidential I/O 与 RISC-V 章节补 lifecycle/state-machine 表，并加入 Arm CCA vs RISC-V CoVE/AP-TEE claim-strength comparison，明确 E0/E1 claim 与 E3 draft direction。 | `arm_cca_spec`, `li2022cca`, `riscv_ap_tee_2024`, `riscv_cove_io_2026`, `rats_rfc`, `eat_rfc`, `dmtf_spdm_2025` |
...
- PCI-SIG TDISP XT metadata: https://pcisig.com/tdisp-extended-tee-xt-extensions
- Accelerator TEE baselines: https://www.usenix.org/conference/osdi18/presentation/volos, https://www.usenix.org/conference/nsdi20/presentation/hunt, https://www.usenix.org/conference/osdi23/presentation/mai, https://arxiv.org/abs/2103.03500, https://dl.acm.org/doi/10.1145/3719653, https://www.researchwithrutgers.com/en/publications/sgx-fpga-trusted-execution-environment-for-cpu-fpga-heterogeneous, and https://arxiv.org/abs/2503.16612
...
- Arm CCA Draft/not ratified expansion: https://arxiv.org/abs/2506.05129 and https://arxiv.org/abs/2512.01594
