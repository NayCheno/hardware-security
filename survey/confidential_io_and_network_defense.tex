...
Attack-only work such as HECKLER shows why this boundary matters: malicious
interrupt delivery can undermine confidential VMs, but this survey keeps
HECKLER in the excluded attack bibliography and uses it only as motivation for
interrupt ownership requirements.\footnote{\url{https://www.usenix.org/conference/usenixsecurity24/presentation/schluter};
metadata is kept in \texttt{survey/excluded\_attack\_reference.bib}.}
...
distributed systems~\cite{giantsidi2025tnic}. \textbf{OSMOSIS} adds a
background resource-management view: multi-tenant SmartNIC kernels contend for
on-NIC compute, DMA, egress bandwidth, and local memory, so a programmable
offload endpoint also needs scheduling and QoS mechanisms before it can be
operationally usable in a multi-tenant setting~\cite{khalilov2024osmosis}.
These works are not substitutes for VM memory protection, SPDM, or TDISP. They
clarify what a NIC or DPU must provide if it becomes a real endpoint of a
...
The \textbf{Security Protocol and Data Model (SPDM; DMTF DSP0274 v1.4.0,
selected as this survey's SPDM snapshot)} provides a protocol for device authentication,
measurement retrieval, version and capability negotiation, and session
establishment~\cite{dmtf_spdm_2025,dmtf_spdm_arch_2024}.
...
Secured Messages Using SPDM (DMTF DSP0277 v2.0.0) adds confidentiality and
integrity for protocol exchanges after session
establishment~\cite{dmtf_spdm_secured_messages_2025}. SPDM over TCP (DMTF
DSP0287 v1.0.0) extends the transport setting so that SPDM can be used beyond
only local bus-oriented interactions~\cite{dmtf_spdm_tcp_2024}. These
specifications do not themselves define a complete confidential I/O
architecture. Their role is to supply the identity, measurement, and
secure-channel substrate that higher level mechanisms such as TDISP, CoVE-IO,
and trusted I/O designs can rely on.
...
confidential workload~\cite{pcisig_tdisp_2022}. The cited PCI-SIG material is a
public ECN/metadata page whose download target is member-gated, so this survey
uses it only for TDISP's public existence, affected-specification metadata, and
device-interface lifecycle role rather than for unreleased normative details
such as exact messages, state-machine rules, or conformance requirements.
The later public \textbf{TDISP eXtended TEE (XT)} ECN metadata is treated under
the same E5 boundary: it supports only the public fact that PCI-SIG lists IDE,
ATS, and TDISP as affected PCIe 6.x areas, not gated XT/TE normative
rules~\cite{pcisig_tdisp_xt_2025}.
Device identity alone is not enough. The platform also needs a way to bind a
particular device interface, function, or trusted device interface to the
workload that will use it. TDISP therefore belongs at the device-interface
lifecycle layer, whereas SPDM belongs at the identity and secure-session layer.
...
\textbf{aDNS} addresses the neighboring discovery and naming problem: how a
client learns that a domain name is bound to a confidential service whose
attestation satisfies that domain's policy. Transparent Attested DNS requires a
TEE-backed service to present hardware attestation before registering keys and
obtaining certificates for names in the relevant zone, and records policies,
attestations, and certificates in a public append-only log~\cite{delignatlavaud2025adns}.
This complements TLS+RA rather than replacing it. TLS+RA binds a channel to an
attested endpoint; aDNS binds service names, certificate issuance, and verifier
policy to attestation evidence. Neither mechanism constrains DMA, assigns a
device interface, or proves the application-level semantics of the service.
...
integrity and data encryption for PCIe traffic, which is link protection rather
than a complete trusted-I/O stack~\cite{pcie_ide}. CXL changes the memory
hierarchy by allowing memory expansion, pooling, and tiering beyond local
DRAM~\cite{cxl_spec,gouk2022directcxl,zhong2024cxltiers}. RDMA and remote paging
systems move application memory over the network or fabric with minimal CPU
involvement~\cite{wang2025odrp}. These CXL/RDMA/fabric sources are background
substrate and data-path evidence, not proof that a confidential-computing
boundary remains intact by themselves; they define paths that a
...
\centering
\scriptsize
\caption{Confidential I/O Lifecycle and State-Machine Summary}
\label{tab:confidential_io_lifecycle}
\begin{tabular}{@{}p{0.15\linewidth}p{0.25\linewidth}p{0.24\linewidth}p{0.26\linewidth}@{}}
\toprule
\textbf{Phase} & \textbf{Representative state transition} & \textbf{Primary evidence status} & \textbf{Survey claim boundary} \\ \midrule
Endpoint identity and session & Requester/responder capabilities, certificates, measurements, and secured-session setup & E0: DMTF DSP0274 v1.4.0 selected SPDM snapshot; secured-message and TCP bindings are protocol substrate~\cite{dmtf_spdm_2025,dmtf_spdm_secured_messages_2025,dmtf_spdm_tcp_2024} & Supports device identity, measurement, negotiation, and secure sessions; does not by itself assign the device or constrain DMA. \\
Interface admission & Device interface is requested, configured, and bound to a protected workload or device manager & E3: CoVE-IO v0.3.0 draft; E5: TDISP/TDISP XT public metadata only~\cite{riscv_cove_io_2026,pcisig_tdisp_2022,pcisig_tdisp_xt_2025} & Trusted-device-interface lifecycle is a draft/metadata-backed direction here; no gated TDISP normative state rule is claimed. \\
DMA and MMIO authority & I/O page tables, SMMU/IOMMU mappings, or IOPMP-style filters are installed for the assigned endpoint & E0: Arm SMMU and RISC-V IOMMU substrate; E3: IOPMP draft; E1: sIOPMP system paper~\cite{arm_smmu_spec,riscv_iommu_2023,riscv_iopmp_2026,feng2024siopmp} & Strong access-control/translation claim for the cited primitive, not a complete endpoint-trust claim. \\
Link and fabric protection & PCIe/CXL/RDMA/NVMe-oF path is encrypted, integrity protected, or routed through a protected data-path design & Spec and background substrate for PCIe IDE/CXL plus E1/E3 systems such as FOLIO, ODRP, and Hazel~\cite{pcie_ide,cxl_spec,li2024folio,wang2025odrp,chrapek2026hazel} & Supports data-path/fabric risk analysis; must be composed with endpoint identity and memory authority before becoming trusted I/O. \\
Interrupt and event delivery & Completion/error/MSI events are delivered to the intended protected domain without host confusion & E0: AIA/IMSIC interrupt substrate; E3/E1 research direction for trusted MSI and device-interrupt protection~\cite{riscv_aia_2023,riscv_cove_io_2026,bertschi2026devlore} & Claim is currently strongest as a required lifecycle layer; complete production semantics remain platform-specific. \\
Operation and re-attestation & Endpoint state, workload channel, or service channel is rechecked while secrets are in use & E0 RATS/EAT roles and token format; E1 TLS+RA/channel-binding evidence; E4 vendor DPU roots~\cite{rats_rfc,eat_rfc,weinhold2025tlsra,nvidia_bluefield_operation_2025} & Evidence must be bound to the workload channel; vendor building blocks are not complete confidential-networking TEEs. \\
Teardown and cleanup & Interface is stopped, DMA authority revoked, device state reset or zeroized, and evidence invalidated & E1 accelerator/offload systems and E3 CoVE-IO direction~\cite{zhu2020hetee,dhar2024cloudscale,riscv_cove_io_2026} & Cleanup is necessary for lifecycle safety but remains an open cross-device validation gap. \\ \bottomrule
\end{tabular}
\end{table*}
...
The broader accelerator TEE lineage shows several different protection routes:
Graviton adds GPU secure contexts and command-processor ownership checks,
Telekine makes cloud-GPU streams independent of secret-dependent behavior,
Honeycomb uses static validation to shrink GPU TEE hardware requirements, and
ShEF moves the same question to cloud FPGAs with secure boot, attestation, and
a configurable Shield~\cite{volos2018graviton,hunt2020telekine,mai2023honeycomb,zhao2022shef}.
XpuTEE is tracked as a recent CPU/GPU heterogeneous TEE baseline, but only with
source-limited metadata and abstract-level evidence in this survey; SGX-FPGA is
also kept as source-limited metadata and is not used for strong mechanism
claims~\cite{fan2025xputee,xia2021sgxfpga}. The SoC-FPGA SoK is useful for
classifying edge and SoC-FPGA TEE gaps, but its claims still need primary
system papers for mechanism detail~\cite{perkins2024socsok}.
...
protection building blocks~\cite{nvidia_bluefield_operation_2025}. That vendor
evidence should be used carefully, because the cited BlueField documentation
describes fTPM support and its limitations rather than a complete production
confidential networking TEE, and it does not by itself bind a workload channel,
DMA policy, or TDISP/SPDM lifecycle to the DPU state.

S-NIC, TNIC, and OSMOSIS fill three additional gaps. S-NIC answers how a
SmartNIC can isolate tenant network functions from other functions and from the
NIC OS, TNIC answers how a NIC can provide a small, verifiable trust root for
networked protocols, and OSMOSIS answers how a SmartNIC can multiplex
compute, DMA, egress, and memory resources among tenants with explicit QoS
policies~\cite{zhou2024snic,giantsidi2025tnic,khalilov2024osmosis}. Hazel then shows how DPU/SmartNIC
...
attestation, protocol-level non-equivocation, resource-management QoS, and
storage freshness.
...
Attested service channel & Does the application channel terminate in the attested protected endpoint? & TLS+RA, aDNS, RA-TLS-style endpoint binding, verifier policy & Bind service names, certificates, KBS endpoints, proxies, and DPU-hosted TLS termination to TEE evidence. \\
...
NIC-local root and function isolation & Can a SmartNIC/DPU isolate tenant functions, multiplex resources, or provide a trusted network primitive? & S-NIC, TNIC, OSMOSIS resource-management substrate, vendor DPU OP-TEE/fTPM building blocks & Treat NICs and DPUs as explicit but incomplete trust endpoints that still need attestation, lifecycle, DMA, channel binding, and resource policy. \\
...
Device-local confidential execution & Does the accelerator or offload engine hold secrets or execute trusted code? & ACAI, PORTAL, CAGE, StrongBox, Graviton, Telekine, Honeycomb, ShEF, ITX, XpuTEE source-limited metadata, SGX-FPGA source-limited metadata, HETEE, CloudScale, vendor BlueField OP-TEE/fTPM building blocks, accelerator TEE designs & Extend or avoid extending the TCB to accelerators and offload devices; do not treat vendor building blocks or source-limited metadata as complete confidential-networking TEEs. \\ \bottomrule
\end{tabular}
\end{table*}
...
device-interface lifecycle concepts, with TDISP cited here only through public
metadata rather than gated normative state-machine rules, and CoVE-IO still a
draft. TLS+RA-style protocols bind application channels to attested endpoints,
while aDNS adds attested naming, certificate issuance, and service-policy
discovery.
SMMU, IOMMU, IOPMP, and sIOPMP constrain DMA
...
may cross, but they are background substrate unless composed with device
identity, lifecycle, and DMA controls. Accelerator, SmartNIC, NIC, and DPU work
such as ACAI, PORTAL, CAGE, StrongBox, ITX, HETEE, CloudScale, S-NIC, TNIC,
OSMOSIS, Hazel, and BlueField OP-TEE/fTPM shows that device-local, NIC-local,
storage-path, or controller-mediated execution can contribute building blocks
for a confidential boundary. A survey that focuses only on CPU TEE mechanisms
would miss this entire class of
