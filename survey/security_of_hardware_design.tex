...
Recent systems evaluation shows that these primitives should still be treated as partial isolation building blocks rather than complete security boundaries. Chen et al. compare Intel MPK, Arm MTE, Arm PAC, and Arm Morello for fine-grained subsystem isolation, and find that practical low-overhead isolation also depends on software transparency, efficient zero-copy exchange, cross-core synchronization of access rights, enough isolation domains, and first-class revocation support~\cite{chen2024hardwareisolation}. This result is useful for the survey's hardware-design lane because it turns MTE, PAC, and capability-style mechanisms from a feature checklist into an engineering question: can the mechanism preserve an isolation boundary under frequent sharing, revocation, and cross-core execution?

\subsubsection{RISC-V Control-Flow and Protection Primitives}

The RISC-V architectural material surveyed here shows a related, though more modular, direction. The privileged and associated architectural specifications document control-flow integrity capabilities through extensions such as \textbf{Zicfilp}, which constrains indirect-control targets through landing-pad semantics, and \textbf{Zicfiss}, which provides hardware support for protecting return-address state~\cite{riscv_privileged}. CVA6-CFI provides implementation and evaluation evidence for those RISC-V CFI extensions in one processor context, but it is not evidence of a complete TEE, memory-confidentiality, DMA-isolation, or CoVE/AP-TEE boundary~\cite{manoni2026cva6cfi}. Rather than embedding one vertically integrated protection model, the RISC-V ecosystem exposes a set of composable primitives that can be incorporated into different classes of systems.
...
Recent architecture work also shows that these layers can be deliberately
co-designed rather than simply stacked. \textbf{Voodoo} combines memory tagging,
authenticated encryption, and DRAM error correction through a MAGIC-style
construction, using tag-dependent authentication to protect encrypted memory
while also preserving correction capabilities~\cite{lamster2024voodoo}. Its
role in this survey is narrow but important: it is evidence that memory-safety
metadata, confidentiality, integrity, and reliability can share a hardware
primitive. It is not evidence for Realm or TVM lifecycle state, attestation, DMA
authorization, or trusted device assignment.
...
Verified enclave monitors are an important intermediate point in that lineage.
\textbf{Komodo} shows how enclave-management policy can be moved out of fixed
hardware and into a machine-checked software monitor while relying on smaller
hardware primitives for isolation, memory protection, and attestation
support~\cite{ferraiuolo2017komodo}. Its role here is not to stand in for Arm
CCA or CoVE/AP-TEE, but to show why verification and small monitor interfaces
matter for TEE design.
...
Runtime attestation adds another layer: the verifier may need evidence about
the path a program actually took, not only the code image that booted.
\textbf{C-FLAT} introduces control-flow attestation with TrustZone-protected
measurement, \textbf{LO-FAT} moves that measurement into hardware to avoid
software instrumentation overhead, and \textbf{VRASED} demonstrates a formally
verified hardware/software RA co-design for simple embedded devices
~\cite{abera2016cflat,dessouky2017lofat,nunes2019vrased}. These papers do not
replace RATS/EAT or platform-specific CCA/CoVE evidence profiles; they explain
why runtime evidence and root-of-trust implementation details matter.
...
Attestation is not only a protocol feature; it is also a hardware-design concern because the architecture must expose stable roots of trust, measurable state, and protection domains whose claims can be verified. The \textbf{RATS} architecture standardizes attestation roles and conceptual messages, and the \textbf{EAT} standard provides a general structure for carrying attestation claims; neither replaces platform-specific evidence semantics~\cite{rats_rfc,eat_rfc}. Broader work on attestation mechanisms for TEEs emphasizes that the usefulness of those claims depends on what the hardware and trusted runtime are able to measure and report~\cite{menetrey2022attestation}. In this sense, attestation belongs in a survey of hardware security design because it links platform mechanisms to external trust decisions.
...
Forward-edge control-flow protection & BTI & Zicfilp; CVA6-CFI prototype evidence \\
Return-address protection & GCS / PAC-assisted hardening & Zicfiss; CVA6-CFI prototype evidence \\
