...
- BibTeX key: mccune2008flicker
- Category: trusted-execution-environments
- Authors: Jonathan M. McCune; Bryan J. Parno; Adrian Perrig; Michael K. Reiter; Hiroshi Isozaki
- Year: 2008
- Venue: EuroSys 2008
- Source: https://doi.org/10.1145/1352592.1352625
- PDF source: https://www.cs.wm.edu/~smherwig/readings/papers/08-eurosys-flicker.pdf
- Local PDF: paper.pdf
- Download status: downloaded and verified
- Survey lane: Arm/RISC-V confidential-computing defense
- Evidence role: 基础/历史入口：pre-SGX/pre-CCA dynamic root of trust and fine-grained attestation
...
- 论文标题: Flicker: An Execution Infrastructure for TCB Minimization
- 作者 / 机构: Jonathan M. McCune; Bryan J. Parno; Adrian Perrig; Michael K. Reiter; Hiroshi Isozaki
- 发表会议 / 年份: EuroSys 2008, 2008
- 领域分类: 架构 / 安全 / 系统
- 一句话总结: Flicker 利用 AMD/Intel late launch 和 TPM attestation，让很小的 security-sensitive code block 在恶意 OS、BIOS 和 DMA 设备假设下隔离执行。
- 最核心贡献一句话: late launch -> measured isolated execution -> TPM quote -> remote verifier
...
- 方法/系统: Flicker: An Execution Infrastructure for TCB Minimization
- 架构管线: late launch -> measured isolated execution -> TPM quote -> remote verifier
...
论文报告 AMD 平台实现，强调约 250 行额外受信代码；性能受当时 TPM/late-launch 开销限制。 评估结论可用于 lineage 和机制比较；跨年代性能数字不能直接和现代 CCA/CoVE/TDX/SEV-SNP 系统横向比较。
...
适合说明极小 TCB 和细粒度 attestation 的历史起点，但不是现代 confidential VM 或 device TEE 的完整运行时。 不能把该论文的结论扩展到 out-of-scope side-channel、fault、Rowhammer 或现代 production deployment。
...
- Evidence 定位: 基础/历史入口：pre-SGX/pre-CCA dynamic root of trust and fine-grained attestation
...
Flicker 利用 AMD/Intel late launch 和 TPM attestation，让很小的 security-sensitive code block 在恶意 OS、BIOS 和 DMA 设备假设下隔离执行。
...
late launch -> measured isolated execution -> TPM quote -> remote verifier

#### 实验结果

论文报告 AMD 平台实现，强调约 250 行额外受信代码；性能受当时 TPM/late-launch 开销限制。
