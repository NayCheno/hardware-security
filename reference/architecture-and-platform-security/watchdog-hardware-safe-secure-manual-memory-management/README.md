...
- BibTeX key: nagarakatte2012watchdog
- Category: architecture-and-platform-security
- Authors: Santosh Nagarakatte; Milo M. K. Martin; Steve Zdancewic
- Year: 2012
- Venue: ISCA 2012
- Source: https://doi.org/10.1109/ISCA.2012.6237017
- PDF source: https://www.cis.upenn.edu/~stevez/papers/NMZ12.pdf
- Local PDF: paper.pdf
- Download status: downloaded and verified
- Survey lane: ISA/hardware-design defense
- Evidence role: 基础/历史入口：hardware temporal plus spatial memory safety
...
- 论文标题: Watchdog: Hardware for Safe and Secure Manual Memory Management and Full Memory Safety
- 作者 / 机构: Santosh Nagarakatte; Milo M. K. Martin; Steve Zdancewic
- 发表会议 / 年份: ISCA 2012, 2012
- 领域分类: 架构 / 安全 / 系统
- 一句话总结: Watchdog 用 allocation identifiers、shadow metadata、micro-ops 和 metadata cache 检测 use-after-free 与 bounds errors。
- 最核心贡献一句话: allocation ID generation -> pointer metadata propagation -> dereference checks -> identifier/bounds validation
...
- 方法/系统: Watchdog: Hardware for Safe and Secure Manual Memory Management and Full Memory Safety
- 架构管线: allocation ID generation -> pointer metadata propagation -> dereference checks -> identifier/bounds validation
...
论文报告 ISCA 2012 hardware design 与性能评估，用于补齐 temporal safety lineage。 评估结论可用于 lineage 和机制比较；跨年代性能数字不能直接和现代 CCA/CoVE/TDX/SEV-SNP 系统横向比较。
...
需要体系结构和编译/runtime 支持；不是 CCA/TEE 隔离机制。 不能把该论文的结论扩展到 out-of-scope side-channel、fault、Rowhammer 或现代 production deployment。
...
- SOTA 定位: 基础/历史入口：hardware temporal plus spatial memory safety
- 标准化 / 发表状态: peer-reviewed venue
- 对应小方向: ISA / 硬件设计防御: MTE、PTE/page table、CFI、capability/tagging。

#### 内容摘要

Watchdog 用 allocation identifiers、shadow metadata、micro-ops 和 metadata cache 检测 use-after-free 与 bounds errors。
...
allocation ID generation -> pointer metadata propagation -> dereference checks -> identifier/bounds validation

#### 实验结果

论文报告 ISCA 2012 hardware design 与性能评估，用于补齐 temporal safety lineage。
