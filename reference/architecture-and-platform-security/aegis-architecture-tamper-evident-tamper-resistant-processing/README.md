...
- Year: 2003
- Venue: ICS 2003
- Source: https://doi.org/10.1145/782814.782838
- PDF source: https://people.csail.mit.edu/devadas/pubs/aegis.pdf
- Local PDF: paper.pdf
- Download status: downloaded and verified
- Survey lane: ISA/hardware-design defense
- Evidence role: 基础/历史入口：single-chip secure processor and untrusted off-chip memory
...
- 论文标题: AEGIS: Architecture for Tamper-Evident and Tamper-Resistant Processing
- 作者 / 机构: G. Edward Suh; Dwaine Clarke; Blaise Gassend; Marten van Dijk; Srinivas Devadas
- 发表会议 / 年份: ICS 2003, 2003
- 领域分类: 架构 / 安全 / 系统
- 一句话总结: AEGIS 把 processor chip 作为信任边界，探索 tamper-evident/tamper-resistant execution 和 untrusted memory。
- 最核心贡献一句话: secure processor mode -> program measurement -> protected storage -> memory encryption/integrity support
...
- 方法/系统: AEGIS: Architecture for Tamper-Evident and Tamper-Resistant Processing
- 架构管线: secure processor mode -> program measurement -> protected storage -> memory encryption/integrity support
...
论文给出两种实现方向和模拟评估，是 secure processor lineage 起点。 评估结论可用于 lineage 和机制比较；跨年代性能数字不能直接和现代 CCA/CoVE/TDX/SEV-SNP 系统横向比较。
...
早期 secure processor 架构，不是现代 VM ownership/access-control 规范。 不能把该论文的结论扩展到 out-of-scope side-channel、fault、Rowhammer 或现代 production deployment。
...
- SOTA 定位: 基础/历史入口：single-chip secure processor and untrusted off-chip memory
- 标准化 / 发表状态: peer-reviewed venue
- 对应小方向: 硬件辅助 TEE 总体设计空间；Memory encryption / integrity / replay protection。

#### 内容摘要

AEGIS 把 processor chip 作为信任边界，探索 tamper-evident/tamper-resistant execution 和 untrusted memory。
...
secure processor mode -> program measurement -> protected storage -> memory encryption/integrity support

#### 实验结果

论文给出两种实现方向和模拟评估，是 secure processor lineage 起点。
