...
根据公开元数据和相关公开文档，只能抽象出高层机制路径: `TSM/TVM policy -> SPDM/CMA control channel -> DSM/TDI lifecycle -> IDE protected interconnect -> trusted attach/detach -> DMA/MMIO policy`。关键概念包括 TEE Device Interface、Device Security Module、TEE Security Manager、SPDM、IDE 和 TDI lifecycle；精确消息和状态规则不能从 gated ECN 推断。
