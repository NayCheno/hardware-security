...
RISC-V 的开放 ISA 让 enclave/TEE 研究快速分化到 PMP monitor、tagged memory、scalable memory protection、confidential VM、embedded confidential computing 等路线。论文解决读者缺少综述入口的问题。证据源为 MDPI HTML 与已验证的 MDPI static resource PDF；该文是公开 peer-reviewed survey，因此证据类型为 E2，具体机制 claims 应回引本仓库已下载的原始论文和规范。
...
作为 survey，不提供新证明或攻击复现。价值在横向分类；风险是不同系统 threat model 不一致，不能把 embedded enclave、PMP-based enclave 和 confidential VM 混成同一安全等级。当前 README 已标明 MDPI HTML 与本地 PDF 均已核验，但不把 survey 当作一手机制证据。
...
最大局限是 survey 对新 draft spec 的状态可能滞后，且横向综述不能替代逐篇机制核验。另一个风险是把 RISC-V 的 open hardware flexibility 过度解读为安全性保证；安全仍取决于 TCB、monitor、内存系统、I/O 和 attestation。
...
- Evidence role: Background substrate. Survey/background substrate for RISC-V enclave and TEE taxonomy; mechanism claims must be traced to original papers or official RISC-V sources.
- 标准化 / 发表状态: peer-reviewed Electronics 2025; MDPI HTML and static-resource PDF verified
...
Survey，无新实验；本地证据为 MDPI HTML 页面和已验证的本地 PDF。
