...
- Download status: attempted again on 2026-05-12; MDPI final PDF endpoint and Preprints.org PDF endpoint returned HTTP 403 to automated download, MDPI HTML source verified
- Evidence class: E2 peer-reviewed survey
- Evidence role: Background substrate. Survey/background substrate for RISC-V enclave and TEE taxonomy; mechanism claims must be traced to original papers or official RISC-V sources.
...
RISC-V 的开放 ISA 让 enclave/TEE 研究快速分化到 PMP monitor、tagged memory、scalable memory protection、confidential VM、embedded confidential computing 等路线。论文解决读者缺少综述入口的问题。证据源为 MDPI HTML；PDF 自动下载被 403 阻挡，但该文是公开 peer-reviewed survey，因此证据类型为 E2，具体机制 claims 应回引本仓库已下载的原始论文和规范。
...
作为 survey，不提供新证明或攻击复现。价值在横向分类；风险是不同系统 threat model 不一致，不能把 embedded enclave、PMP-based enclave 和 confidential VM 混成同一安全等级。当前 README 已标明 PDF 自动下载受阻，但不再把公开 peer-reviewed survey 降为 E5。
...
- 标准化 / 发表状态: peer-reviewed Electronics 2025; MDPI HTML verified, automated PDF download returned 403
...
Survey，无新实验；本地证据为 MDPI HTML 页面，PDF endpoint 自动下载 403。
