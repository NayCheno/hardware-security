...
{\large\bfseries\color{Ink} Confidential I/O protocol、trusted device interface、network endpoint 的核心是先界定保护对象、管理边界和证据强度，再用三篇主讲材料串起技术演进。\par}\vspace{1.8mm}
...
Separate but Together: TLS channel binding with remote attestat。 & 当前 SOTA/补充边界 & E1 / Peer-reviewed endpoint channel-binding evidence \\
\bottomrule
\end{tabularx}
}
    \end{minipage}
...
{\large\bfseries\color{Ink} SPDM 定义 device/component identity、measurement、capability negotiation、cert。\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
    ...
    \hspace{1.4mm}\begin{minipage}{0.92\linewidth}
      \PanelTitle{内容说明}
      \scriptsize \begin{itemize}
  \item SPDM 定义 device/component identity、measurement、capability negotiation、certificate/challenge。
  \item SPDM is the first-party specification foundation for device identity, measurement, and secu。
\end{itemize}
    \end{minipage}
    \vspace{1mm}
  \end{minipage}}
...
\DeckKicker{研究背景}
\SlideMeta{证据等级}{E0 / Spec-standard SOTA}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} Confidential workload 使用真实设备时，不能只相信 host 枚举出的 PCIe requester ID、VF 配置或驱动状态\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
\DeckKicker{解决方案}
\SlideMeta{证据等级}{E0 / Spec-standard SOTA}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} 通过 requester/responder 协议、certificate chain、measurement blocks、transcript。\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
    ...
    \hspace{1.4mm}\begin{minipage}{0.92\linewidth}
      \PanelTitle{内容说明}
      \scriptsize \begin{itemize}
  \item 通过 requester/responder 协议、certificate chain、measurement blocks、transcript 和 secured session。
\end{itemize}
    \end{minipage}
    \vspace{1mm}
  \end{minipage}}
...
\DeckKicker{实验结果}
\SlideMeta{证据等级}{E0 / Spec-standard SOTA}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} 规范/Survey，无新实验；SPDM device identity and measurement 只支撑术语、taxonomy、接口语义或证据边界。\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
  ...
  \item 可支撑：SPDM is the first-party specification foundation for device identity, measurement, an。
...
\DeckKicker{文章评价}
\SlideMeta{证据等级}{E0 / Spec-standard SOTA}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} 评价：SPDM device identity and measurement 适合做 taxonomy/规范入口；规范/Survey，无新实验，不能替代一手系统证据。\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
  ...
  \item 商业化潜力：适合 fleet-scale device attestation；风险在证书供应链、measurement schema 和 firmware update polic。
...
\textbf{维度} & \textbf{判断} \\
\midrule
设计优势 & 是 CoVE-IO、TDISP、PCIe IDE keying 和 vendor device attestation 的共同 device-evidence。 \\
主要局限 & 单独成功的 SPDM session 不能证明 TDI state、IOMMU/IOPMP policy、IDE link 或 device firmware。 \\
商业落地 & 适合 fleet-scale device attestation；风险在证书供应链、measurement schema 和 firmware update。 \\
...
{\large\bfseries\color{Ink} CoVE-IO 把 TVM trusted I/O 拆成 TDI/TDM/DSM、SPDM、TDISP、PCIe IDE、IOMMU、truste。\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
\DeckKicker{研究背景}
\SlideMeta{证据等级}{E3 / 草案/未批准规范}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} TVM/Realm 使用设备时，需要把 device interface identity、DMA/MMIO、interrupt delivery。\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
\DeckKicker{解决方案}
\SlideMeta{证据等级}{E3 / 草案/未批准规范}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} 用 CoVE-IO draft 描述 RISC-V confidential I/O lifecycle\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
\DeckKicker{实验结果}
\SlideMeta{证据等级}{E3 / 草案/未批准规范}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} 规范/Survey，无新实验；CoVE-IO and TDISP trusted-device lifecycle 只支撑术语、taxonomy、接口语义或证据边界。\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
\DeckKicker{文章评价}
\SlideMeta{证据等级}{E3 / 草案/未批准规范}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} 评价：CoVE-IO and TDISP trusted-device lifecycle 适合做 taxonomy/规范入口；规范/Survey，无新实验，不能替代一手系统证据。\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
  ...
  \item 商业化潜力：指向 vendor NIC/DPU/accelerator assignment；风险在 SPDM证书、TDISP state、IDE key、ATS/PRI 和 rev。
  \item 规范/Survey，无新实验；具体平台结论转到一手材料核验。
\end{itemize}
    \end{minipage}
    \vspace{1mm}
...
商业落地 & 指向 vendor NIC/DPU/accelerator assignment；风险在 SPDM证书、TDISP state、IDE key、ATS/PRI。 \\
讲稿定位 & 代表性改进; 证据强度 E3 \\
\bottomrule
\end{tabularx}
}
...
{\large\bfseries\color{Ink} TLS+RA binds an application secure channel to an attested TEE endpoint, anchoring endpoint/channel-binding evid。\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
\end{columns}
\vspace{1mm}
{\tiny\textcolor{Muted}{引用依据：引用依据为论文原文、官方规范或公开材料；本页结论按 E1 / Peer-reviewed endpoint channel-binding evidence 的证据等级使用，不外推到未验证平台。}}
\end{frame}
...
\DeckKicker{研究背景}
\SlideMeta{证据等级}{E1 / Peer-reviewed endpoint channel-binding evidence}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} SPDM and TDISP can identify device/control-plane state, but they do not prove where an application TLS channel。\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
保护对象 & Confidential I/O protocol、trusted device interface、network endpoint \\
传统瓶颈 & 把 SPDM/TDISP/PCIe IDE/attested TLS 与 trusted device lifecycle 组合起来写。 \\
本文入口 & Separate but Together: TLS channel binding with remote attestat。 \\
...
\DeckKicker{解决方案}
\SlideMeta{证据等级}{E1 / Peer-reviewed endpoint channel-binding evidence}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} Integrate remote-attestation evidence into the TLS handshake or certificate path so the channel and TEE state a。\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
\DeckKicker{实验结果}
\SlideMeta{证据等级}{E1 / Peer-reviewed endpoint channel-binding evidence}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} USENIX ATC 2025 peer-reviewed system paper provides a TLS+RA prototype and evaluation.\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
\textbf{对象} & \textbf{可支撑} & \textbf{边界} \\
\midrule
数据/来源 & Separate but Together: TLS channel binding with remote attestat。 & 本地 PDF 已核验 \\
Baseline/对照 & 以论文或规范原文为准 & 不跨材料外推 \\
...
\DeckKicker{文章评价}
\SlideMeta{证据等级}{E1 / Peer-reviewed endpoint channel-binding evidence}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} Fits the protocol/device-endpoint 主讲定位 better than FOLIO, which is a trusted-I/O contrast paper.\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
    ...
    \hspace{1.4mm}\begin{minipage}{0.92\linewidth}
      \PanelTitle{内容说明}
      \scriptsize \begin{itemize}
  \item 设计优势：Fits the protocol/device-endpoint 主讲定位 better than FOLIO, which is a trusted-I/O cont。
  \item 局限性：TLS+RA does not solve device assignment, TDISP state, IDE link protection, or IOMMU/I。
  \item 商业化潜力：Useful for confidential service onboarding and key release; risks remain in certifica。
...
\textbf{维度} & \textbf{判断} \\
\midrule
设计优势 & Fits the protocol/device-endpoint 主讲定位 better than FOLIO, which is a trusted-I/。 \\
主要局限 & TLS+RA does not solve device assignment, TDISP state, IDE link protection, or I。 \\
商业落地 & Useful for confidential service onboarding and key release; risks remain in cer。 \\
...
\DeckKicker{方向总结}
\SlideMeta{证据等级}{方向综述}{来源状态：公开材料综合}
{\large\bfseries\color{Ink} Confidential I/O protocol、trusted device interface、network endpoint 的结论是：三篇主讲材料共同给出技术演进，但仍要用证据等级约束每个结论。\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
  ...
  \item Separate but Together: TLS channel binding with remote attestat。：当前 SOTA/补充边界，E1 / Peer-reviewed endpoint channel-binding evidence。
...
\midrule
SPDM device identity and measurement & 开创/基础入口 & SPDM is the first-party specification foundation for device identity, mea。 \\
CoVE-IO and TDISP trusted-device lifecycle & 代表性改进 & RISC-V CoVE-IO draft maps SPDM and TDISP-style ideas into the RISC-V TEE-。 \\
Separate but Together: TLS channel binding with remote attestat。 & 当前 SOTA/补充边界 & Peer-reviewed endpoint/channel-binding evidence for integrating remote at。 \\
\bottomrule
\end{tabularx}
}
    \end{minipage}
