...
{\large\bfseries\color{Ink} Arm TrustZone TEE 与漏洞谱系 的核心是先界定保护对象、管理边界和证据强度，再用三篇主讲材料串起技术演进。\par}\vspace{1.8mm}
...
{\large\bfseries\color{Ink} 介绍 Secure/Non-secure world、NS bit 和 SoC 级隔离的基本模型\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
  ...
  \item First-party Arm TrustZone whitepaper is the strongest foundational mechanism source for thi。
\end{itemize}
    \end{minipage}
    \vspace{1mm}
  \end{minipage}}
...
\end{columns}
\vspace{1mm}
{\tiny\textcolor{Muted}{引用依据：引用依据为论文原文、官方规范或公开材料；本页结论按 E4 / 基础产业证据 的证据等级使用，不外推到未验证平台。}}
\end{frame}
...
\DeckKicker{研究背景}
\SlideMeta{证据等级}{E4 / 基础产业证据}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} 移动和嵌入式平台需要把安全服务从普通 OS 中隔离出来\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
\DeckKicker{解决方案}
\SlideMeta{证据等级}{E4 / 基础产业证据}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} 在处理器、互连和外设路径传播安全状态，形成双世界系统\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
\DeckKicker{实验结果}
\SlideMeta{证据等级}{E4 / 基础产业证据}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} 白皮书/industry evidence，无独立实验\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
\DeckKicker{文章评价}
\SlideMeta{证据等级}{E4 / 基础产业证据}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} 历史基础价值高，能解释 Arm TEE 的双世界起点\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
{\large\bfseries\color{Ink} 全面梳理 TrustZone 架构、TEE 软件栈、应用和虚拟化相关工作\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
\DeckKicker{研究背景}
\SlideMeta{证据等级}{E2 / Background substrate}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} TrustZone 长期被广泛部署，但公开系统化资料不足，学术系统和产业 TEE 栈边界不统一\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
\DeckKicker{解决方案}
\SlideMeta{证据等级}{E2 / Background substrate}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} 按系统软件、TEE、虚拟化和应用场景分类，给 TrustZone 生态建立 survey map\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
\DeckKicker{实验结果}
\SlideMeta{证据等级}{E2 / Background substrate}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} 规范/Survey，无新实验；Demystifying Arm TrustZone 只支撑术语、taxonomy、接口语义或证据边界。\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
  ...
  \item 可支撑：Systematic survey of TrustZone-assisted TEEs for mature mobile and embedded TEE linea。
...
\DeckKicker{文章评价}
\SlideMeta{证据等级}{E2 / Background substrate}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} 评价：Demystifying Arm TrustZone 适合做 taxonomy/规范入口；规范/Survey，无新实验，不能替代一手系统证据。\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
  ...
  \item 局限性：对 CCA/RME 不完整，不能直接支撑 Realm 或 confidential VM 结论。
...
主要局限 & 对 CCA/RME 不完整，不能直接支撑 Realm 或 confidential VM 结论。 \\
...
{\large\bfseries\color{Ink} 归纳 TrustZone-assisted TEE 的主流漏洞类型和根因\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
\end{columns}
\vspace{1mm}
{\tiny\textcolor{Muted}{引用依据：引用依据为论文原文、官方规范或公开材料；本页结论按 E2 / Peer-reviewed SoK 的证据等级使用，不外推到未验证平台。}}
\end{frame}
...
\DeckKicker{研究背景}
\SlideMeta{证据等级}{E2 / Peer-reviewed SoK}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} TrustZone TEE 暴露接口复杂、TCB 变大，真实系统反复出现漏洞\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
\DeckKicker{解决方案}
\SlideMeta{证据等级}{E2 / Peer-reviewed SoK}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} 按漏洞面、接口、权限边界和实现缺陷建立 taxonomy，解释为什么后续 CCA 需要更强边界\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
\DeckKicker{实验结果}
\SlideMeta{证据等级}{E2 / Peer-reviewed SoK}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} 规范/Survey，无新实验；TrustZone-assisted TEE 漏洞 SoK 只支撑术语、taxonomy、接口语义或证据边界。\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
\DeckKicker{文章评价}
\SlideMeta{证据等级}{E2 / Peer-reviewed SoK}{来源状态：本地 PDF 已核验}
{\large\bfseries\color{Ink} 评价：TrustZone-assisted TEE 漏洞 SoK 适合做 taxonomy/规范入口；规范/Survey，无新实验，不能替代一手系统证据。\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
\DeckKicker{方向总结}
\SlideMeta{证据等级}{方向综述}{来源状态：公开材料综合}
{\large\bfseries\color{Ink} Arm TrustZone TEE 与漏洞谱系 的结论是：三篇主讲材料共同给出技术演进，但仍要用证据等级约束每个结论。\par}\vspace{1.8mm}
\begin{columns}[T,totalwidth=\textwidth]
  \begin{column}{0.45\textwidth}
    \fcolorbox{Rule}{Paper}{%
  \begin{minipage}[t]{0.97\linewidth}
...
\midrule
Arm TrustZone 白皮书 & 开创/基础入口 & First-party Arm TrustZone whitepaper is the strongest foundational mechan。 \\
Demystifying Arm TrustZone & 代表性改进 & Systematic survey of TrustZone-assisted TEEs for mature mobile and embedd。 \\
TrustZone-assisted TEE 漏洞 SoK & 当前 SOTA/补充边界 & S\&P 2020 SoK anchors the TrustZone vulnerability and attack-surface narra。 \\
\bottomrule
\end{tabularx}
}
    \end{minipage}
